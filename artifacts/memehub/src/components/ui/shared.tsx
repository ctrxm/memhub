...
export function Badge({ className, variant = "default", children, onClick }: { className?: string, variant?: "default"|"secondary"|"outline", children: React.ReactNode, onClick?: () => void }) {
  const variants = {
    default: "border-transparent bg-primary text-primary-foreground hover:bg-primary/80",
    secondary: "border-transparent bg-secondary text-secondary-foreground hover:bg-secondary/80",
    outline: "text-foreground border-border"
  };
  const cls = cn("inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2", variants[variant], onClick && "cursor-pointer select-none", className);
  if (onClick) {
